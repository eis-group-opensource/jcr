...
public class EffectiveNodeType implements Cloneable {
    private static Log log = LogFactory.getLog(EffectiveNodeType.class);
    ...
    static EffectiveNodeType create(NodeTypeDef ntd,
        ...
        for (int i = 0; i < cnda.length; i++) {
            ...
            if (itemDefIds.containsKey(cnda[i].getNodeDefId())) {
                // conflict
                String msg;
                ...
                itemDefIds.put(cnda[i].getNodeDefId(), cnda[i]);
            }
            if (cnda[i].definesResidual()) {
        ...
        for (int i = 0; i < pda.length; i++) {
            ...
            // this node type definition
            if (itemDefIds.containsKey(pda[i].getPropDefId())) {
                // conflict
                String msg;
                ...
                itemDefIds.put(pda[i].getPropDefId(), pda[i]);
            }
            if (pda[i].definesResidual()) {
    ...
    public NodeDef getApplicableChildNodeDef(QName name, QName nodeTypeName,
        ...
        throw new ConstraintViolationException("no matching child node definition found for " + name +" of type "+nodeTypeName+" in "+toString());
    }
    ...
    public PropDef getApplicablePropertyDef(QName name, int type,
        ...
        if (type == PropertyType.REFERENCE && match == null){
            match =
                getMatchingPropDef(getNamedPropDefs(name), PropertyType283.WEAKREFERENCE, multiValued);
        }
        if (match != null) {
            return match;
        }
    ...
    public String toString() {
        ...
        builder.append("nodetypes", this.allNodeTypes);
        builder.append("namedItems", namedItemDefs);
        builder.append("unnamedItems", unnamedItemDefs);
