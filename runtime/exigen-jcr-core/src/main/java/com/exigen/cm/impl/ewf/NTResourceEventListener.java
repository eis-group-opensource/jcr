...
public class NTResourceEventListener implements BeforeSaveEventListener{
        ...
        while(events.hasNext()){
            ...
            try {
            	if (log.isDebugEnabled()){
            		log.debug("NTREsourceEventListener : "+e.getPath()+" event type: "+e.getType());
            	}
                ...
                if (!processedNodes.contains(nId)){
                	...
                	if (!nState.getStatus().equals(ItemStatus.Invalidated) && !nState.getStatus().equals(ItemStatus.Destroyed)){
	                    ...
	                    if (!nState.hasProperty(getLastModifiedDateProperty())){
	                        InternalValue value = InternalValue.create(time);
	                        nState.internalSetProperty(getLastModifiedDateProperty(), value, true, false, true);
	                    }
                	}
	...
	private QName getLastModifiedDateProperty() {
		// jcr:lastModified
		return new QName(QName.NS_JCR_URI,"lastModified");
	}
...
/*
 * $Log: NTResourceEventListener.java,v $
 * Revision 1.3  2008/07/09 06:40:24  dparhomenko
 * *** empty log message ***
 *
 * Revision 1.2  2008/03/28 13:45:58  dparhomenko
 * Autmaticaly unlock orphaned locks(session scoped)
 *
 * Revision 1.1  2007/10/19 13:45:18  dparhomenko
