 ...
 */
public class RepositoryImportHandler extends DefaultHandler {

    private static Log log = LogFactory.getLog(RepositoryImportHandler.class);

    protected final RepositoryImporter importer;
    ...
    private HashMap<String,Long> workspaces = new HashMap<String, Long>();
    ...
    public RepositoryImportHandler(Importer importer, NamespaceResolver nsResolver,
                         NamespaceRegistryImpl nsReg) throws RepositoryException {
        this.importer = (RepositoryImporter)importer;
        this.nsResolver = nsResolver;
        this.nsReg = nsReg;
        
        this.repository = ((RepositoryImporter)importer).getRepository();
    ...
    public void startPrefixMapping(String prefix, String uri)
        ...
        try {
            ...
            try {
            	Namespace nn = nsReg._getByURI(uri);
            	if (nn == null){
    ...
    public void startElement(String namespaceURI, String localName, String qName,
        ...
        if (initialized && targetHandler == null){
    		try {
	        	if (localName.equals("system")){
	        		importer.setWorkspaceId(null);
	        		targetHandler = new SysViewImportHandler(importer, nsContext);
	        		targetHandler.startDocument();
	        	}
	        	if (localName.equals("workspace")){
	        		String workspaceName = atts.getValue("name");
	        		Long workspaceId = repository.createWorkspace(conn, nsReg, this.nodeTypeManager ,workspaceName);
	        		conn.commit();
	        		importer.setWorkspaceId(workspaceId);
	        	...
	        	if (localName.equals("workspace-security")){
	        		String workspaceName = atts.getValue("name");
	        		//Long workspaceId = workspaces.get(workspaceName);
	        		
	        		SessionImpl s = repository.createTrustedSession(workspaceName, "DSUSER", conn);
	        		targetHandler = new SecurityImport(s, repository);
	        		targetHandler.startDocument();
	        	}
        ...
        // delegate to target handler
        if (!qName.equals(JCR_ROOT) && targetHandler != null && depth > 2) {            
    ...
    public void endElement(String namespaceURI, String localName, String qName)
        ...
        if (depth ==2){
        	if (targetHandler != null){
	        	targetHandler.endDocument();
	        	if (targetHandler instanceof SecurityImport){
	        		((SecurityImport) targetHandler).closeSession();
	        	}
	        	targetHandler = null;
        	}
        }
