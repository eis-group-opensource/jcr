...
/**
 * MS Power point text extractor based on POI
 * 
 */
public class POIPowerPointTextExtractor implements TextExtractor {

    public void extract(String mimeType, InputStream source, Writer target)
                    throws IOException {
        try {
            PowerPointExtractor x = new PowerPointExtractor(source);
            String s = x.getText();
            target.write(s);
        }
        finally {
...
/*
 * $Log: POIPowerPointTextExtractor.java,v $
