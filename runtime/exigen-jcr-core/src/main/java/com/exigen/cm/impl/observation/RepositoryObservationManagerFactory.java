 ...
 */
public final class RepositoryObservationManagerFactory extends EventDispatcher
        implements Task {

    public static final long MAX_WAIT_TIME = 1000; //5 min  
    public static final long SCHEDULE_PERIOD = 1;

    /**
     * Logger instance for this class
     */
    private static final Log log = LogFactory.getLog(RepositoryObservationManagerFactory.class);
     ...
     */
    private Set<RepositoryEventConsumer> activeConsumers = new HashSet<RepositoryEventConsumer>();

    /**
     * Currently active synchronous <code>EventConsumer</code>s for notification.
     */
    private Set<RepositoryEventConsumer> synchronousConsumers = new HashSet();
    private Set<RepositoryEventConsumer> synchronousBeforeConsumers = new HashSet();
	...
	private RepositoryObservationManagerImpl observationManagerImpl;

    /**
     * Creates a new <code>ObservationManagerFactory</code> instance
     * and starts the notification thread deamon.
     */
    public RepositoryObservationManagerFactory(RepositoryImpl repository) throws RepositoryException{
    	//this.repository = repository;
    	this.observationManagerImpl = new RepositoryObservationManagerImpl(this, repository);
        /*notificationThread = new Thread(this, "ObservationManager");
        notificationThread.setDaemon(true);
        notificationThread.start();*/
    	repository.getTaskManager().schedule(this, SCHEDULE_PERIOD);
    }
    ...
    public void removeConsumersByListenerClass(Class<? extends EventListener> ... listenerClasses){
        removeConsumersByListenerClass(synchronousBeforeConsumers, listenerClasses);   
        removeConsumersByListenerClass(synchronousConsumers, listenerClasses);
        removeConsumersByListenerClass(activeConsumers, listenerClasses);
    }

    void removeConsumersByListenerClass(Set<RepositoryEventConsumer> consumers, Class<? extends EventListener> ... listenerClasses){
        Iterator<RepositoryEventConsumer> consumersIterator = consumers.iterator();
//        boolean done = false;
        Set<RepositoryEventConsumer> targets = new HashSet<RepositoryEventConsumer>();
        while(consumersIterator.hasNext()){
            
            RepositoryEventConsumer consumer = consumersIterator.next();
            Class<? extends EventListener> clClass = consumer.getEventListener().getClass();
            
            for(Class<? extends EventListener> lClass: listenerClasses){
                if(lClass == clClass){
     ...
     */
    public RepositoryObservationManagerImpl getObservationManager() {
    ...
    public void run() {
        ...
        try {
	        while ((action = (DispatchAction) eventQueue.remove(MAX_WAIT_TIME - (System.currentTimeMillis() - start))) != DISPOSE_MARKER 
	        		) {
	            ...
	            for (Iterator it = action.getEventConsumers().iterator(); it.hasNext();) {
	            	RepositoryEventConsumer c = (RepositoryEventConsumer) it.next();
	                try {
     ...
     */
    synchronized void prepareEvents(EventStateCollection events) {
     ...
     * {@inheritDoc}
     */
    synchronized void prepareDeleted(EventStateCollection events, ChangeLog changes) {
        Set consumers = new HashSet();
        consumers.addAll(getSynchronousConsumers());
        consumers.addAll(getAsynchronousConsumers());
        for (Iterator it = consumers.iterator(); it.hasNext();) {
     ...
     */
    synchronized void dispatchEvents(EventStateCollection events, SessionImpl originalSession) {
        // notify synchronous listeners
        Set<RepositoryEventConsumer> synchronous = getSynchronousConsumers();
        if (log.isDebugEnabled()) {
            log.debug("notifying " + synchronous.size() + " synchronous listeners.");
        }
        for (RepositoryEventConsumer c: synchronous) {
            try {
    ...
    synchronized void dispatchBeforeEvents(EventStateCollection events,SessionImpl originalSession) throws RepositoryException {
        // notify synchronous listeners
        ...
        for (Iterator it = synchronous.iterator(); it.hasNext();) {
        	Object obj = it.next();
        	RepositoryEventConsumer c = (RepositoryEventConsumer) obj;
            c.consumeEvents(events, true, new Throwable(), originalSession);
        }
    }
     ...
     */
    synchronized void addConsumer(RepositoryEventConsumer consumer) {
        synchronized (consumerChange) {
     ...
     */
    synchronized void removeConsumer(EventConsumer consumer) {
        synchronized (consumerChange) {
            if (consumer.getEventListener() instanceof BeforeSaveEventListener) {
                synchronousBeforeConsumers.remove(consumer);
