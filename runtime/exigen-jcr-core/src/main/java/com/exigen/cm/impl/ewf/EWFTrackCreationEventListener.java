...
public class EWFTrackCreationEventListener implements BeforeSaveEventListener{
        ...
        while(events.hasNext()){
            ...
            try {
            	if (log.isDebugEnabled()){
            		log.debug("EWFTrackCreationEventListener : "+e.getPath()+" event type: "+e.getType());
            	}
                ...
                if (!processedNodes.contains(nId)){
                	...
                	if (!nState.getStatus().equals(ItemStatus.Invalidated) && !nState.getStatus().equals(ItemStatus.Destroyed)){
	                    ...
	                    if (!nState.hasProperty(getCtreatedDateProperty())){
	                        InternalValue value = InternalValue.create(time);
	                        nState.internalSetProperty(getCtreatedDateProperty(), value, true, false, true);
	                    }
                	}
	...
	protected QName getCtreatedDateProperty() {
		return Constants.EWF_TRACKABLE__CREATED;
	}
...
/*
 * $Log: EWFTrackCreationEventListener.java,v $
 * Revision 1.2  2008/07/09 06:40:24  dparhomenko
 * *** empty log message ***
 *
